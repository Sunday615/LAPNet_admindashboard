...
<template>
  <div class="page tech">
    ...
    <!-- ✅ Success Alert -->
    <Teleport to="body">
      <div v-if="successAlert.open" class="modalOverlay" @click.self="closeSuccessAlert">
        <div class="modalCard">
          <div class="modalTop">
            ...
            <button class="modalX" type="button" @click="closeSuccessAlert" title="Close">
              <i class="fa-solid fa-xmark"></i>
            </button>
          </div>

          <div class="modalBody">
            <div class="modalMsg">ບັນທຶກຂໍ້ມູນສຳເລັດ</div>

            <div class="modalInfo">
              <div class="infoRow">
                <span class="k">Employee:</span>
                <span class="v">{{ successAlert.empName || "-" }}</span>
              </div>
              <div class="infoRow">
                <span class="k">Department:</span>
                <span class="v">{{ form.department || "-" }}</span>
              </div>
              <div class="infoRow">
                <span class="k">Position:</span>
                <span class="v">{{ form.row || "-" }}</span>
              </div>
            </div>
          </div>

          <div class="modalActions">
            <button class="btn ghost" type="button" @click="closeSuccessAlert">
              <i class="fa-solid fa-xmark"></i> Close
            </button>

            <button class="btn primary" type="button" @click="viewInserted">
...
function openSuccessAlert({ empId, empName }) {
  successAlert.value = { open: true, empId: empId ?? null, empName: empName ?? "" };
}
function closeSuccessAlert() {
  successAlert.value.open = false;
}
function viewInserted() {
  const id = successAlert.value.empId;
  closeSuccessAlert();
  router.push({ path: "/lapnetview", query: { highlight: String(id || "") } });
}
...
// ✅ reset after success: (ยังคงเหมือนเดิม) เคลียร์ชื่อ+รูป แต่คง role/department/row ใน session
...
async function onSubmit() {
  ...
  try {
    ...
    fd.append("create_at", form.timestamp);
    ...
    const empId = row?.emp_id ?? row?.id ?? payload?.id ?? null;
    const empName = row?.name ?? form.empName;

    showToast("success", "Saved successfully");
    gsap.to(actionsEl.value, { y: -2, duration: 0.18, yoyo: true, repeat: 1, ease: "power2.out" });

    resetAfterSuccess();
    openSuccessAlert({ empId, empName });
  } catch (e) {
    console.error(e);
...
onMounted(() => {
  // ✅ ไม่โหลดค่า template ใดๆ ตอนเริ่มต้น/refresh (แก้บัคตามที่ขอ)
...
.modalTitle i { color: rgba(34, 197, 94, 0.95); }
.modalX {
  width: 40px;
  height: 40px;
  border-radius: 14px;
