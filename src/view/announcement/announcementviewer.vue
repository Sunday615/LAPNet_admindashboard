<!-- AnnouncementView.vue (UPDATED: ✅ remove local sidebar, use GlobalSidebar from App.vue + ✅ scope GSAP so it won't hide global sidebar) -->
<template>
  <div ref="rootEl" class="page tech">
    <!-- Ambient glow (scoped to this page, not fixed to viewport) -->
    <div class="glow glow-a"></div>
    <div class="glow glow-b"></div>

    <!-- Main (App.vue provides GlobalSidebar) -->
                  ...
                  <div v-for="item in flatEntries" :key="item.k" class="kv">
                    ...
                    <div v-else-if="item.type === 'link'" class="v">
                      <div v-if="item.href" class="linkRow">
                        ...
                        <div class="linkFull mono" :title="item.href">{{ item.href }}</div>
                      </div>
                      <span v-else>-</span>
                    </div>
...
/* ✅ root scope for GSAP (fix: do not touch GlobalSidebar in App.vue) */
...
const tableCols = computed(() => {
  ...
  const MAX_COLS = 8;
  const MAX_PICKED = MAX_COLS - 2; // reserve announcementNo + Link
  ...
  if (t === "string" || t === "number" || t === "boolean") {
    ...
    if (arr) {
      out.push({
        ...
        type: "list",
        items: arr
          .map((x) => (x == null ? "-" : ["string", "number", "boolean"].includes(typeof x) ? String(x) : JSON.stringify(x)))
...
async function fetchAnnouncements() {
  try {
    ...
    // ✅ scoped reveal (no global selector)
...
/* ✅ GSAP context scoped to rootEl (fix global sidebar not showing) */
let gsapCtx = null;
