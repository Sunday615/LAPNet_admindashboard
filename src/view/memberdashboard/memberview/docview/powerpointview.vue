<!-- ExcelViewer.vue (same layout + same table style as Word page) -->
<template>
  <div class="docViewer">
    <!-- Top bar -->
    ...
    <div class="layout">
      ...
      <section class="leftCol">
        ...
        <!-- ✅ Excel Table (Excel only) - SAME STYLE AS WORD -->
        <div ref="excelCardRef" class="card section">
          <div class="wordHead">
            <div class="wordTitle">
              ...
              <div class="wordSub">
                Showing <b>{{ excelDocs.length }}</b> files
                <span class="dot">•</span>
                Sort: <b>{{ sortLabel }}</b>
              </div>
            </div>
            ...
            <div class="wordActions">
              ...
              <button class="btn mini" type="button" @click="createNewExcel">
            ...
            <table class="wordTable">
              <thead>
                <tr>
                  ...
                  <th style="width: 120px">Sheets</th>
              ...
              <tbody v-if="excelDocs.length">
                <tr
                  v-for="d in excelDocs"
                  :key="d.key"
                  class="wordRow"
                ...
                >
                  <td>
                    <div class="wIcon">
                      <img src="/document_icon/powerpoint.png" alt="" />
                    </div>
                  </td>
                  ...
                  <td>
                    <div class="nameCell">
                      <div class="nameTop">
                        <button class="linkBtn" type="button" @click="openExcel(d)">
                        ...
                        <span v-if="d.shared" class="badge2 bShare">
                  ...
                  <td>
                    <div class="sheetsCell">
                      <span class="sheetBadge">
                        <i class="fa-solid fa-table-cells"></i>
                        {{ Number(d.sheetCount || 0) }}
                      </span>
                      <span class="sheetHint">{{ d.primarySheet || "—" }}</span>
                    </div>
                  </td>
                  ...
                  <td style="text-align: right">
                    <div class="rowActions">
                      <button class="iconBtn" type="button" title="Preview" @click="previewExcel(d)">
                        <i class="fa-solid fa-eye"></i>
                      </button>
                      <button class="iconBtn" type="button" title="Share" @click="shareExcel(d)">
                      ...
                      <div class="menuWrap" :ref="(el) => setKeyRef(menuWrapMap, d.key, el)">
                        ...
                        <div class="rowMenu" :ref="(el) => setKeyRef(menuElMap, d.key, el)" role="menu" aria-label="Row menu">
                          <button class="rmItem" type="button" @click="renameExcel(d)">
                            <i class="fa-solid fa-pen"></i><span>Rename</span>
                          </button>
                          <button class="rmItem" type="button" @click="moveExcel(d)">
                            <i class="fa-solid fa-folder-tree"></i><span>Move</span>
                          </button>
                          <button class="rmItem" type="button" @click="downloadExcel(d)">
                            <i class="fa-solid fa-download"></i><span>Download</span>
                          </button>
                          <div class="rmDivider"></div>
                          <button class="rmItem danger" type="button" @click="deleteExcel(d)">
              ...
              <tbody v-else>
                <tr>
                  <td colspan="8">
                    <div class="empty">
                      <div class="emptyIcon"><i class="fa-solid fa-file-excel"></i></div>
                      <div class="emptyTitle">No Excel documents found</div>
      ...
      <aside class="rightCol">
        ...
        <!-- Recent Sheets -->
        <div ref="sheetsCardRef" class="card section">
          <div class="sectionTitle">Recent Sheets</div>

          <div class="list">
            <div v-for="s in recentSheets" :key="s.key" class="listRow">
              <div class="rowLeft">
                <div class="sheetIcon">
                  <i class="fa-solid fa-table-cells"></i>
                </div>
...
/**
 * ✅ key ref (stable for sort/filter)
 */
...
// ---------------------
// Excel documents (filter/sort) + helpers
// ---------------------
const documents = ref([
  {
    key: "x1",
    ...
    size: 4100000,
    sheetCount: 6,
    primarySheet: "Summary",
    updatedAt: "2026-01-19T03:25:00Z",
    owner: "Finance",
    tags: ["Sales", "2023"],
    path: "/excel/Sales2023.pptx",
    locked: false,
    star: true,
    shared: true,
  },
  {
    ...
    sheetCount: 4,
    primarySheet: "Q1",
    updatedAt: "2026-01-18T09:12:00Z",
    owner: "John",
    tags: ["Budget", "Q1"],
    path: "/excel/Budget-Q1.pptx",
    locked: true,
    star: false,
    shared: false,
  },
  {
    ...
    owner: "Warehouse",
    tags: ["Inventory"],
    path: "/excel/Inventory.pptx",
    locked: false,
    star: false,
    shared: false,
  },

  // non-excel examples (won't show)
  { key: "p1", name: "Narrative-Report.pdf", size: 580000, updatedAt: "2026-01-10T08:00:00Z", owner: "John", tags: ["PDF"], path: "/pdf/Narrative-Report.pdf" },
  { key: "w1", name: "Report101.docx", size: 1048576, updatedAt: "2026-01-18T09:12:00Z", owner: "John", tags: ["Word"], path: "/docs/Report101.docx" },
]);
...
function setSort(k) {
  sortKey.value = k;
  closeRowMenu(true);
  nextTick().then(animatepowerpointRows);
}

const excelDocs = computed(() => {
  const needle = String(q.value || "").trim().toLowerCase();

  const onlyExcel = documents.value.filter((d) => isExcelFile(d.name));

  const filtered = !needle
    ? onlyExcel
    : onlyExcel.filter((d) => {
        const hay = `${d.name} ${d.path} ${d.owner} ${(d.tags || []).join(" ")} ${d.primarySheet || ""}`.toLowerCase();
...
watch(
  () => excelDocs.value.map((x) => x.key).join("|"),
  async () => {
    closeRowMenu(true);
    await nextTick();
    animateExcelRows();
...
// actions (demo)
function openExcel(d) {
  closeRowMenu();
  showToast(`Open: ${d.name}`);
}
function previewExcel(d) {
  closeRowMenu();
  showToast(`Preview: ${d.name}`);
}
function shareExcel(d) {
  closeRowMenu();
...
function renameExcel(d) {
  closeRowMenu();
  showToast(`Rename: ${d.name}`);
}
function moveExcel(d) {
  closeRowMenu();
  showToast(`Move: ${d.name}`);
}
function deleteExcel(d) {
  closeRowMenu();
...
function createNewExcel() {
  const now = new Date();
  const key = `x_${Math.random().toString(16).slice(2)}`;
  documents.value.unshift({
    key,
    name: `New-Excel-${now.getTime()}.xlsx`,
    size: 180000,
    sheetCount: 1,
    primarySheet: "Sheet1",
    ...
    owner: "John",
    tags: ["Draft"],
    path: `/excel/New-Excel-${now.getTime()}.xlsx`,
    locked: false,
    star: false,
    shared: false,
  });
  showToast("Created new Excel file");
}

// ---------------------
// right widgets + animations
// ---------------------
...
const recentSheets = ref([
  { key: "s1", name: "Summary", meta: "Sales2023.xlsx • Updated today" },
  { key: "s2", name: "Q1", meta: "Budget-Q1.xlsx • Updated yesterday" },
]);

const recentFiles = ref([
  { key: "f1", name: "Sales2023.xlsx", size: "4.1MB" },
  { key: "f2", name: "Budget-Q1.xlsx", size: "820KB" },
  { key: "f3", name: "Inventory.xls", size: "560KB" },
]);
...
function animateExcelRows() {
  const reduce = window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches;
  if (reduce) return;

  const rows = excelDocs.value.map((d) => rowElMap.value?.[String(d.key)]).filter(Boolean);
...
onMounted(async () => {
  ...
  if (!reduce) {
    ...
    tl.from([docTypeCardRef.value, excelCardRef.value, externalCardRef.value], { y: 14, opacity: 0, duration: 0.55, stagger: 0.10 }, 0.05);
    tl.from([internalCardRef.value, sheetsCardRef.value, filesCardRef.value], { y: 14, opacity: 0, duration: 0.55, stagger: 0.12 }, 0.12);
